...
void Measure(){
	...
	for (int i=0; i<npart-1; ++i){
	    for (int j=i+1; j<npart; ++j){
			...
			if (dr < box/2)
				walker[int(dr * nbins * 2. / box) + igofr] += 2;
...
void Averages(int iblk){ //Print results for current block
	...
	for (int i = igofr; i < igofr + nbins; i++){
		deltaV = pi / 24. * pow(box / nbins,3) * (1. + 12. * pow(i - 1, 2));
		stima_gdir = blk_av[i]/(blk_norm * rho * npart * deltaV);
		Gofr << setw(wd) << stima_gdir;
	...
	if(iblk == nblk){
		...
		for (int i = igofr; i < igofr + nbins; i++){
			...
			Gave << glob_av[i]/(double)iblk << setw(wd) << err_gdir << endl;
		}
		Gave.close();
	}
...
void ValoriIstantanei(void){

	int M = 1000; // Numero degli step da fare
	ofstream Epot, Pres;
	
	Epot.open("output.epot.insta");
	Pres.open("output.pres.insta");
	
	Reset(1);	// Metti i contatori a zero
	for (int i = 0; i < M; i++){
		Move();
		Measure();
		Epot << walker[iv]/double(npart) + vtail << endl; //Potential energy per particle
		Pres << rho * temp + (walker[iw] + ptail * double(npart))/vol << endl; //Pressure
		if (((i+1)*20)%M == 0){
			cout << "Completato il " << ((i+1)*100)/M << "%" << endl;
			//cout << "Acceptance rate " << accepted/attempted << endl << endl;
		}
	}
