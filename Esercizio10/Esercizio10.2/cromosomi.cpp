...
void Cromosoma::Configurazione(const vector <vector<double>> &città, int rank){

	ofstream output;
	output.open("posizioni_città" + to_string(rank) + ".txt");
...
Random generatore(int rank){ 	// Prepare the random number generator
...
void PrintCost(const vector <Cromosoma> &cromosomi, int ngen, int rank){
	...
	// Stampa i risultati
	ofstream output;
	output.open("generazioni" + to_string(rank) + ".txt", ios::app);
...
vector <Cromosoma> Crossover(const Cromosoma &padre, const Cromosoma &madre, double prob, Random &rand){
	...
	if (rand.Rannyu() < prob){
		...
		// altrimenti scambio soltanto madre con padre
...
void Scambia(int quanti, int rank, vector <Cromosoma> &cromo, Random &rand){
	...
	int n_città = cromo[0].GetGeni().size();
	int index;
	
	// Scegli un processo a caso con cui scambiare
	if (rank == 0)
		index = int(rand.Rannyu()*3) + 1;
	
	// Fallo sapere agli altri processi e stabilisci con chi scambiare
	MPI_Bcast(&index, 1, MPI_INTEGER, 0, MPI_COMM_WORLD);
	if (rank != 0){
		int num;
		if (rank == index)
			num = 0;
			...
			for (int i = 1; i < 4; i++)
				if (i != index and i != rank)
					num = i;
		}
		index = num;
	}

	// Ciclo su quanti cromosomi voglio scambiare
	for (int i = 0; i < quanti; i++){
			
		vector <int> imesg1(cromo[i].GetGeni());
		vector <int> imesg2(n_città);
		MPI_Request req;
		
		// Scambio gli elementi
		int itag = rank * quanti + i;
		MPI_Isend(&imesg1[0], n_città, MPI_INTEGER, index, itag, MPI_COMM_WORLD, &req);
		MPI_Recv(&imesg2[0], n_città, MPI_INTEGER, index, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		cromo[i] = Cromosoma(imesg2);
	}
	
	// Non la cosa ottimale riordinare tutto il vettore per mettere a posto solo
	// qualche elemento. In ogni caso il programma è veloce quindi non mi preoccupo
	// di questo.
	Ordina(cromo);
}
